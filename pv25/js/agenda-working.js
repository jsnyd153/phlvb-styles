...
// New logic to get names from formatted categoryItems
    ...
    // Helper function to find category names by title
function getCategoryNamesByTitle(categoryItems, title) {
    const category = categoryItems.find(c => c.title === title);
    if (category) {
        return category.items.map(item => {
            // Use a regular expression to match and replace anything in parentheses
...
// This function now fetches AND formats the data.
function fetchAndFormatSessions() {
  ...
  .then(function(response) {
    ...
    formattedSessionsData = newFormattedData; // Store globally
    console.log(formattedSessionsData)
    
    // Return the formatted data and other top-level arrays
    return {
...
// ... All other functions go here (getTargetAudienceNames, createSessionItem, etc.) ...
// No changes needed for these functions, but they will now use the formattedSessionsData.

// The fsAttributes callback is the main entry point
  ...
  function(filterInstances) {
    ...
    fetchAndFormatSessions()
      ...
      .then(function() {
        ...
        //Manually adding. these fro now since dynamically creating them isn't workign. with fsattributes
        // const targetAudienceFiltersHtml = createCategoryFilters('Target Audience', 'filter-audience');
        // $('#audience-dropdown').append(targetAudienceFiltersHtml);
        // const targetTypeFiltersHtml = createCategoryFilters('Session format', 'filter-type');
        // $('#type-dropdown').append(targetTypeFiltersHtml);
        // const targetLevelFiltersHtml = createCategoryFilters('Level', 'filter-level');
        // $('#level-dropdown').append(targetLevelFiltersHtml);
...
function createCategoryFilters(categoryTitle, categoryFilterName) {
    ...
    function addClickSessionTrigger(){
        $('a[modal-trigger="session-modal"]').click(function(e){
            ...
            const sessionId = href.split('#session-').pop();
             console.log('Modal trigger clicked', href, sessionId);
    ...
    $('.session_modal-speakers').empty().append(speakerElementsHtml);
        ...
        // Replace each match with an HTML <a> tag
	...
	if (brandHoverCards.length > 0) {
		brandHoverCards.each(function() {
            ...
            if(colorSetting == 'Lightning Talk'){
                colors = ['rgb(84, 80, 224)', 'rgb(114, 176, 254)'];
            }
            if(colorSetting == 'Breakout Session'){
                colors = ['rgba(254, 162, 55, 1)', 'rgba(245, 194, 127, 1)'];
            }
            if(colorSetting == 'Keynote'){
                colors = ['rgba(246, 63, 92, 1)', 'rgba(250, 141, 90, 1)'];
            }
            if(colorSetting == 'Training'){
                colors = ['rgba(126, 44, 191, 1)', 'rgba(183, 116, 243, 1)'];
            }
